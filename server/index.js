...
io.on('connection', function(socket) {
  log('Entering the Socket Connection');
  socket.on('signin_send', (data) => {  
    log('Sign In Send Data is:: ', data);
    // database_config.sql = "SELECT * FROM casino_user;"
    database_config.sql = 'SELECT * FROM casino_user WHERE email = "' + data.user + '";'
    database_config.name = "db04"
      database(database_config).then(function(result){
      log('Sign In Result Object:: ', result)
      ...
      if(result && result[0] && result[1]){
        ...
        if(user_found && user_found.length>0){
          ...
          let user_wallet = getUserWallet(user_found)

          if(!user_wallet)
            user_wallet = createUserWallet(user_found, "carrots", user_found[0].money)

          //Is User Wallet Present
          if(!user_wallet)
            try{
          ...
          try{
            io.to(socket.id).emit('signin_read', {exists: false, obj: {result}})
          }catch(e){
  ...
  socket.on('signup_send', (data) => {  
    ...
    database_config.name = "db04"
      ...
      if(!result){
        ...
        log('I am now in the ELSE block')    
  ...
  function getUserWallet(userEmail){
    //Fetch User Wallet From DB
    database_config.sql += "SELECT * FROM user_wallet WHERE user_email = '" + userEmail + "';"
    database_config.name = "db04"

    database(database_config).then(function(result){ 
      if(result) // User has a Wallet
        return result
  ...
  function createUserWallet(user_found, currency = "usd", balance = 0.0 ){
    //insert new wallet for user
    let uuid = crypto.randomBytes(20).toString('hex')

    database_config.sql = "INSERT INTO user_wallet (uuid, user, email, currency, balance) VALUES (?, ?, ?, ?, ?)"

    let payload = [uuid, user_found.user, user_found.email, currency, balance]
